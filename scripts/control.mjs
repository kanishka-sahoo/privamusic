...
else if(action==='discover')result=await fetch(`${base}/api/discover/check`,{method:'POST',headers,body:'{}'});
else result=await fetch(`${base}/api/state`,{headers});
const data=await result.json();
if(!result.ok)throw Error(data.error);
if(action==='discover')console.log(JSON.stringify({created:data.created.map(j=>({id:j.id,feed:j.feed,edition:j.edition}))},null,2));
else if(action==='status')console.log(JSON.stringify({connected:data.connected,navReady:data.navReady,halted:data.halted,discover:data.discover,jobs:data.jobs.map(j=>({id:j.id,name:j.name,kind:j.kind,status:j.status,done:j.done,failed:j.failed,total:j.tracks.length,playlistId:j.playlistId,error:j.error,trackErrors:j.tracks.filter(t=>t.error).map(t=>({name:t.name,error:t.error}))}))},null,2));
