...
export async function verifyAudio(path,expectedMs=0){
  const {stdout}=await exec(process.env.FFPROBE_PATH||'/root/.local/share/spotiflac-next/ffprobe',['-v','error','-show_entries','format=duration:stream=codec_type,codec_name','-of','json',path],{timeout:30000,maxBuffer:1024*1024});
  ...
  async run(job){
    try{
      ...
      for(const t of job.tracks){
        ...
        try{
          try{Object.assign(t,await verifyAudio(target,t.duration_ms),{status:'completed',error:null});this.save(job);continue;}catch{}
          ...
          try{result=await this.bridge.call('DownloadTrack',[{
            ...
            item_id:`${job.id}-${t.spotify_id}`,allow_fallback:true,embed_lyrics:true,embed_max_quality_cover:true,
