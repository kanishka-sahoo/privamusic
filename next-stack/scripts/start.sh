...
x_pid=$!
trap 'kill "$server_pid" "$x_pid" "${app_pid:-}" "${vnc_pid:-}" "${web_pid:-}" 2>/dev/null || true; wait' TERM INT EXIT
count=0
...
vnc_pid=$!
websockify --web=/usr/share/novnc 127.0.0.1:6080 127.0.0.1:5900 > /data/native-websocket.log 2>&1 &
web_pid=$!
while kill -0 "$server_pid" 2>/dev/null && kill -0 "$app_pid" 2>/dev/null && kill -0 "$vnc_pid" 2>/dev/null && kill -0 "$web_pid" 2>/dev/null; do sleep 2; done
exit 1
