...
export class ListenBrainz {
  ...
  async request(url,body){
    ...
    if(r.status===404)throw Error(url.startsWith(API)?'ListenBrainz user or playlist not found':'ListenBrainz Labs endpoint not found');
  ...
  // Spotify track IDs per recording, aligned with `tracks`: by MBID first, then by artist/release/title.
...
export class Discover {
  ...
  // Worker hook: the feed's recordings as Spotify track metadata. `lookup` resolves one Spotify track URL through the
  ...
  async resolve(job,lookup,known=[]){
    const tracks=await this.client.playlist(job.source.mbid);
    if(!tracks.length)throw Error('ListenBrainz playlist is empty');
    const ids=await this.client.spotifyIds(tracks);
    const byId=new Map(known.map(t=>[t.spotify_id,t])),byIsrc=new Map(known.filter(t=>t.isrc).map(t=>[t.isrc,t]));
    const resolved=[];let lookups=0;
    for(let i=0;i<tracks.length;i++){
      const source=tracks[i],candidates=ids[i];
      if(!candidates.length){resolved.push({spotify_id:null,mbid:source.mbid,name:source.title,artists:source.artist,album_name:source.album,duration_ms:source.duration,status:'skipped',error:'No Spotify match for this recording'});continue;}
      let track=byId.get(candidates.find(id=>byId.has(id)));
      if(!track){
        // Space Spotify lookups through the native app like downloads, but more lightly.
        if(lookups++)await sleep(this.lookupDelay);
        try{track=metadata(await lookup(`https://open.spotify.com/track/${candidates[0]}`),'track').tracks[0];}
