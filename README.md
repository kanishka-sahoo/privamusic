...
- Recordings are mapped to Spotify in four passes: the ListenBrainz Labs lookup by MusicBrainz recording ID, the Spotify links that MusicBrainz editors attached to the recording, the Labs lookup by artist/release/title, and finally the Spotify albums linked from the recording's MusicBrainz releases, searched by ISRC and title through the downloader. Recordings that still have no match are marked **Skipped**, do not count as failures, and are left out of the playlist. **Recheck skipped** on a finished edition (or **Retry** on a partial one) looks for them again.
